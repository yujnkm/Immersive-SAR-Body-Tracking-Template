...
public class SkeletonHandler : ScriptableObject
{
   ...
   // HumanBodyBones.LastBone, // Right Heel
    ...
    [SerializeField] private Vector3 skeletonOffset = new Vector3(0f, 0f, -3f);
    [SerializeField] private float skeletonYawDeg = 37f;
    ...
    {
        // ------------------------------------------------------------------
        // 1) Copy and  offset once so **everyone** sees the same world coords.
        //    We don’t touch the SDK array in case another system re-uses it.
        // ------------------------------------------------------------------
        Quaternion yaw = Quaternion.Euler(0f, skeletonYawDeg, 0f);
        Vector3[] shifted = (currentKeypointsCount == jointsPosition.Length)
                       ? new Vector3[currentKeypointsCount]
                       : (Vector3[])jointsPosition.Clone();     // fallback

        //if (skeletonOffset != Vector3.zero)
        //    for (int i = 0; i < shifted.Length; i++)
        //        shifted[i] = jointsPosition[i] + skeletonOffset;

        for (int i = 0; i < shifted.Length; i++)
        {
            Vector3 translated = jointsPosition[i] + skeletonOffset;
            shifted[i] = yaw * translated;
        }

        rootRotation = yaw * rootRotation;
        currentJoints = shifted;
        
            // Anything below this point must use **currentJoints** (already shifted)
        ...
        usingAvatar = useAvatar;
            ...
            if (useAvatar)
                {
            SetHumanPoseControl(currentJoints[0], rootRotation, jointsRotation, _mirrorOnYAxis);
            
                    // Debug skeleton uses the same data, so no extra offset needed.
                    if (ZEDBodyTrackingManager.DisplaySDKSkeleton)
                UpdateSkeleton(Vector3.zero, _mirrorOnYAxis);
                }
            else
                {
            UpdateSkeleton(Vector3.zero, _mirrorOnYAxis);   // non-avatar path
                }
    }
